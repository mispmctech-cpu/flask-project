...
(function() {
    ...
    async function fetchEnabledMonths() {
        ...
        try {
            ...
            if (error) {
                console.warn('Error fetching enabled months from Supabase:', error);
                return null;
            }
            
            if (data && Array.isArray(data.enabled_months) && data.enabled_months.length > 0) {
                console.log('Fetched enabled months from database:', data.enabled_months);
                return data.enabled_months;
            } else {
                console.warn('No enabled months found in database, returning null for fallback');
                return null;
            }
        } catch (err) {
    ...
    // Check if current page is an institution form
    function isInstitutionForm() {
        const pageURL = window.location.pathname.toLowerCase();
        return pageURL.includes('institution-form');
    }
    ...
    async function filterMonthDropdowns() {
        ...
        console.log('[Month Filter] Is Institution Form:', isInstitutionForm());
        
        // Default fallback: all 12 months
        const allMonths = [
        ...
        if (!enabledMonths || enabledMonths.length === 0) {
            ...
            enabledMonths = allMonths;
            console.log('[Month Filter] Using fallback: all 12 months enabled');
        }

        // For institution forms, always show all enabled months (skip faculty assignment filtering)
        // For faculty forms, apply faculty assignment filtering
        let visibleMonths = enabledMonths;
        
        if (isInstitutionForm()) {
            // Institution forms: show all enabled months, no faculty filtering
            visibleMonths = enabledMonths;
            console.log('[Month Filter] Institution form detected - showing all enabled months:', visibleMonths);
        } else if (assignedMonths !== null && assignedMonths !== undefined) {
            // Faculty forms: apply faculty assignment filtering
            if (assignedMonths.length > 0) {
                ...
                // Faculty has NO months assigned (empty assigned_forms) - show all enabled months as fallback
                visibleMonths = enabledMonths;
                console.log('[Month Filter] Faculty has empty assigned_forms - showing all enabled months as fallback');
            }
        } else {
